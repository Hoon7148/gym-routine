    ...
    <div className="scr pt-2 pb-[120px]">
      ...
      <div className="px-5">
        ...
        <div className="rounded-[20px] overflow-hidden bg-card border border-white/[0.07] mb-3.5">
          ...
          <div className="p-4">
            <div className="text-[22px] font-extrabold text-text tracking-tight leading-tight">푸시 데이 — 상체 볼륨</div>
            <div className="flex items-center gap-1.5 mt-1">
              <span className="w-[5px] h-[5px] rounded-full bg-success" />
              <span className="text-[13px] text-text-dim">CBUM 유튜브 루틴 · 큐레이터 검증</span>
            </div>
            <div className="flex items-center gap-4 mt-3.5">
              <div><span className="mono text-[19px] font-extrabold text-text">8</span><span className="text-xs text-text-dim"> 종목</span></div>
              <div className="w-px h-4 bg-white/10" />
              <div><span className="mono text-[19px] font-extrabold text-text">62</span><span className="text-xs text-text-dim"> 분</span></div>
              <div className="w-px h-4 bg-white/10" />
              <div><span className="mono text-[19px] font-extrabold text-text">1.2k</span><span className="text-xs text-text-dim"> 진행중</span></div>
            </div>
          </div>
        </div>
        ...
        {homeExercises.map((d) => (
          <div key={d.num} onClick={() => openDetail()} className="flex items-center gap-3.5 py-3.5 border-b border-white/[0.06] cursor-pointer">
            ...
            <div className="flex items-center gap-1 bg-accent/[0.12] border border-accent/25 rounded-[9px] px-2.5 py-1.5 flex-none">
              ...
              <span className="mono text-xs font-bold text-accent">{d.time}</span>
            </div>
          </div>
        ))}
