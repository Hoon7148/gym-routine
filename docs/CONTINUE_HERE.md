...
3. ~~**PWA 아이콘**~~ — 완료 (2026-07-06). Explore 화면 '푸시' 카테고리 아이콘의
   ...
   200 응답(404 없음) 확인.
4. **인증** — 아직 시작 전. 로그인 없음, `준경`/아바타 이니셜이 하드코딩. 이게
   있어야 Record/Curator/Profile도 Supabase로 옮길 수 있음. 다음 세션에서
   시작하기 전에 아래 두 가지를 사용자에게 먼저 확인할 것(2026-07-07에 물어봤으나
   아직 답 못 받음):
   - **인증 방식**: OAuth(구글/카카오)는 외부 개발자 콘솔 등록이 별도로 필요해서
     제외하는 걸 제안했음. 대신 Supabase 내장 인증 중 (a) 매직링크(이메일 OTP,
     비밀번호 없음 — 구현 가장 간단, 추천) vs (b) 이메일+비밀번호(더 익숙한 UX,
     회원가입/로그인/비밀번호 재설정 화면 다 필요해서 코드 더 많음) 중 선택 필요.
   - **이번 라운드 범위**: (a) 로그인/로그아웃 + 세션 연결까지만 하고 Record/
     Curator는 이번에도 mockData 유지(추천 — 인증 자체를 먼저 확실히 검증) vs
     (b) Record의 운동 기록까지 실제 `workouts`/`workout_sets`에 쓰도록 한번에
     연결(범위 커지고, 인증+쓰기 둘 다 실제 프로젝트 없이는 검증 불가).
   - 참고로 Supabase 프로젝트가 아직 연결 안 됐다면(`.env.local` 없음) 인증
     작업도 마찬가지로 실제 프로젝트 없이는 로그인 자체를 테스트할 수 없음 —
     아래 "Supabase 프로젝트 연결하기"를 먼저 하는 게 순서상 맞음.
