...
**브라우저 종단 간(end-to-end) 테스트 완료.** claude-in-chrome으로
`localhost:5199`를 직접 열어(실제 로그인 세션 유지된 채로) 전 화면을
클릭 검증함:

- [x] **Curator 실제 쓰기** — "인클라인 바벨 벤치프레스" −5 클릭 →
      화면 3:57→3:52 즉시 반영 → DB 조회로 `timestamp_seconds: 232` 확정.
- [x] **Home 트렌딩** — "오늘 인기 루틴"/"트렌딩 종목"이 실제 workout_sets
      집계값과 정확히 일치하는 것 확인(플랫 덤벨 프레스 2, 인클라인 덤벨
      플라이 1 등).
- [x] **콜드스타트 수동 토글** — "콜드스타트" 버튼 클릭 시 실제
      `is_featured` 루틴 3개(Chris Bumstead/Larry Wheels 검증)로 정상 전환.
- [x] **Explore** — 가슴 부위 선택 시 실제 루틴 2개(푸시 데이, 벤치 특화)
      정확히 필터링됨.
- [x] **RoutineDetail** — Explore에서 "벤치 특화" 클릭 → 실 종목 6개 +
      타임스탬프 정상 표시, 진행자 없는 루틴이라 "— 명 진행"으로 정확히 표시.
- [x] **Record (새 루틴)** — "벤치 특화"로 기록 시작 → 새 workout 자동
      생성 → 세트 추가 → `get_trending_exercises` RPC로 새 세트가 실제
      반영된 것 확인(exercise `...0009` count 1로 등장).

콜드스타트 **자동** 전환(트렌딩 데이터가 0일 때)은 지금은 실제 트렌딩
데이터가 이미 있어서 재현 못 함 — 로직 자체(`if (data.length === 0)
setCold(true)`)는 코드 리뷰로 확인됨, 필요하면 나중에 새 프로젝트로
재현 검증.
